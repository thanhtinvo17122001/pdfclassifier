...
def adv_train(model, train_interval_path, test_interval_path, model_name):
    ...
    x_input = pickle.load(open(os.path.join(train_interval_path, 'x_input.pickle'), 'rb'), encoding="latin1")
    ...
    with tf.Session() as sess:
        ...
        for epoch in range(20):
            ...
            robust_train_batch = [False for k in range(int(len(x_train)/batch_size)+1)] + [True for k in range(int(len(x_input)/batch_size)+1)]
            ...
            for k in range(len(robust_train_batch)):
                ...
                #for cur_batch in range(int(len(x_input) / batch_size)):
                    if i+batch_size > x_input.shape[0]:
                        ...
                        last_x_upper = np.concatenate((vectors_all[i:], vectors_all[:remain_size]))
